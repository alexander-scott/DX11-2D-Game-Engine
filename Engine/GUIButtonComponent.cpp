...
void GUIButtonComponent::Update(float deltaTime)
{
	...
	if (Mouse::Instance().LeftIsPressed() && !mIsPressed)
	{
		...
		Rect buttonRect;
		buttonRect.LeftX = (int)(mTransform->GetWorldPosition().x - (mWidth / 2));
		buttonRect.RightX = (int)(mTransform->GetWorldPosition().x + (mWidth / 2));
		buttonRect.BotY = (int)(mTransform->GetWorldPosition().y - (mHeight / 2));
		buttonRect.TopY = (int)(mTransform->GetWorldPosition().y + (mHeight / 2));
